...
| —— | **host key 校验** —— spike 阶段直接 accept-all，源码注明 `// TODO(v1.2): 由 KnownHostStore 接管`；不算 S1-S6 验收项；**凭据写源码同样禁止** |
...
| 操作 | 客户端行为 |
|------|----------|
| **从 server 拉主机** | 只读 `id / name / hostname / port / username / authType / createdAt / updatedAt`，**忽略** `password / privateKey / keyPassphrase` 字段（即便 server 有返回）|
...
| **本地添加/修改主机 metadata** | 通过 oRPC `sshHost.upsert` 上传，但 payload 中不带凭据字段（即便 server schema 接受）|
| **删除主机** | oRPC 删除 + Keychain 同步删除（按 `caterm.host.<id>.*` 通配） |
