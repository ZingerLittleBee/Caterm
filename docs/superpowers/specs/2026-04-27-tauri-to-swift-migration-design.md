...
│  │  - Auto-reconnect 状态机                        │    │
│  │  - host key 校验 (调 KnownHostStore)            │    │
│  └────┬────────────────┬────────────────┬─────────┘     │
│       │                │                │                │
│  ┌────▼────────┐ ┌─────▼──────┐ ┌──────▼─────────┐      │
│  │ Keychain    │ │ KnownHost  │ │ ConfigStore    │      │
│  │ Store       │ │ Store      │ │ (Ghostty cfg)  │      │
│  │ (凭据)      │ │ (TOFU)     │ │                │      │
│  └─────────────┘ └────────────┘ └────────────────┘      │
│                                                          │
│  ┌─────────────────────────────────────────────────┐    │
...
**关键纪律**：跨边界传字节用 `AsyncThrowingStream<Data, Error>` 包一层 `BoundedByteChannel`（带界容量，详见 §4.2）；`@MainActor` 与 NIO 之间通过 `Task` + `MainActor.run` 显式切换；不允许 NIO callback 直接 touch SwiftUI state。
...
SSHTransport.connect(config)
   ...
   │                        ├─ 存在 + match  → promise.succeed → NIO 继续
   │                        ├─ 存在 + mismatch → promise.fail(HostKeyMismatchError)
   │                        │                    UI 显示警告，连接终止（必须用户手动解决）
   │                        └─ 不存在 → 走 TOFU 异步流程（详见下方 ⓘ）
   │
   │  → auth (password / publickey) → open session channel → request PTY
   │
   ├─ 成功: 返回 Connection { stdoutStream, stdinSink, resizeSink, closeSink }

ⓘ **TOFU 异步流程（NIO event loop 永远不阻塞）**：
   1. SSHTransport 在 hostKeyValidator 内创建 `EventLoopPromise<HostKeyDecision>`
   2. 通过 `HostKeyDecisionProvider`（一个 protocol，由 SessionStore 实现并持有引用）
      把 challenge 派发到 MainActor：`Task { @MainActor in await provider.decide(fingerprint:) }`
   3. UI 弹 TOFU 对话框，用户做选择
   4. provider 把决定（trust / reject）`promise.succeed(...)` 回给 NIO loop
   5. hostKeyValidator 根据 promise 结果决定 NIO future 成败
   ⚠️ NIO event loop 上**严禁** `wait()` / `try await` 等待 UI；只能 promise 模式异步桥接
   │   │
   │   ▼
...
NIOSSH Channel inbound  ──→  BoundedByteChannel  (cap = 256 KB)
    ...
    │                            │  缓冲水位
    │                            ├─ < HIGH (192 KB)  → 正常读
    │                            ├─ ≥ HIGH           → channel.setOption(.autoRead, false) (暂停 NIO 读)
    │                            └─ < LOW  (64 KB)   → channel.setOption(.autoRead, true)；
    │                                                  必要时显式 channel.read() 触发下一轮
    │
...
ghostty write callback(bytes) ──→ SSHTransport.stdinSink.write(bytes)
                                  (Task → NIO loop;有界 64 KB；
                                   超过则按 4 KB 分块 await flush —— paste 防爆)
    │
    ▼
...
2. `BoundedByteChannel` 是有界队列；`cat huge.log` / `yes` 这类高吞吐场景，水位高时关闭 `autoRead`，由 TCP 窗口反向限速远端 —— 不允许 unbounded stream 把内存炸穿
3. 数值（256K/192K/64K）是初始猜测，spike 之后基于实测调整；写进配置常量便于后期改
4. NIOSSH child channel 创建时**必须启用 remote half-closure**：`channel.setOption(ChannelOptions.allowRemoteHalfClosure, true)`（NIOSSH README 写法），否则 EOF 处理不正确
5. **stdin 也有界**：键盘单按键速率极低，但 **paste 可能瞬间几 MB**。`stdinSink` 内部上限 64 KB；超过则按 4 KB 分块写入并 await flush，避免一次性塞爆 channel write buffer
...
- 格式：与 OpenSSH `known_hosts` 兼容
  - 22 端口：`hostname keytype base64key`
  - 非 22 端口：`[hostname]:port keytype base64key`（方括号是 OpenSSH 规范的一部分）
  - 多 key per host 用多行（OpenSSH 标准）
...
| **SSHTransport — 集成** | 真实 auth（password & publickey）、PTY、stdin/stdout、resize、EOF | **Docker `linuxserver/openssh-server` 容器**，本地（开发机/dogfood）运行 `swift test` 时拉起；v1 不要求每次 PR 都跑（与 R5 一致：v1 不上 CI），ship 前手动跑一遍 |
...
1. NIOSSH child channel 必须启用 half-closure：`channel.setOption(ChannelOptions.allowRemoteHalfClosure, true)`（写法见 NIOSSH README），否则 EOF 不正确
...
| **拉到的主机本地无 Keychain 凭据** | 在 SessionStore 给该 host 打标 `needsLocalCredential = true`；UI 列表显示锁图标；用户首次连接时弹"该主机尚未在本机配置凭据"提示，进入"添加凭据"流程 |
| **本地添加/修改凭据** | 仅写 Keychain，**绝不**通过 oRPC 上传到 server |
| **本地新建主机（同步开启时）** | 调用 **`sshHost.create`**（payload 中**不带** `password / privateKey / keyPassphrase`，即便 server schema 接受）；server 返回的 `{ id }` 写入本地 `host.serverId`（详见 §7.1.3 id 映射）|
| **本地修改主机 metadata（已同步）** | 调用 **`sshHost.update`**，`id` 用本地 `host.serverId`，payload 不含凭据字段 |
| **本地删除主机** | 已同步：`sshHost.delete(id: serverId)` + Keychain 按本地 `host.id` 通配删除（`caterm.host.<localId>.*`）；未同步：仅本地（Keychain + JSON）|
...
v1 阶段 `Host.id` 是客户端生成的 UUID，**同时用作 Keychain 主键**（`caterm.host.<id>.password` 等）。v1.1 同步开启后，server `sshHost.create` 会**返回服务端自己生成的 UUID**（见 `packages/api/src/routers/ssh-host.ts:68`），与本地 id 不同。
...
| Server 端有但本地没有（其它设备新建的） | 本地新建 host：客户端生成新 local id 作为 Keychain key 入口；`serverId` 设为 server 返回的 id；标 `needsLocalCredential = true` |
