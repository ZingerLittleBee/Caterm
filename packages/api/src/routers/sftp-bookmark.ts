...
export const sftpBookmarkRouter = {
  list: protectedProcedure.input(z.object({ hostId: z.string().optional() })).handler(async ({ input, context }) => {
  ...
  update: protectedProcedure
    ...
    .handler(async ({ input, context }) => {
      ...
      const result = await db
        .update(sftpBookmark)
        .set(rest)
        .where(and(eq(sftpBookmark.id, id), eq(sftpBookmark.userId, context.session.user.id)))
  ...
  delete: protectedProcedure.input(z.object({ id: z.string() })).handler(async ({ input, context }) => {
    const result = await db
      .delete(sftpBookmark)
      .where(and(eq(sftpBookmark.id, input.id), eq(sftpBookmark.userId, context.session.user.id)))
      .returning({ id: sftpBookmark.id })
    if (result.length === 0) {
      throw new ORPCError('NOT_FOUND', {
        message: 'Bookmark not found'
