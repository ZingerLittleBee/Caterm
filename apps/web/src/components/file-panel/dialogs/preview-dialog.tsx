...
export function PreviewDialog({ onClose, open, path, readFile }: PreviewDialogProps) {
  ...
  const loadContent = useCallback(async () => {
    ...
    setContent(null)
    try {
      const text = await readFile(path, 1024 * 1024)
      setContent(text)
    } catch (err) {
    ...
    <Dialog.Root onOpenChange={(isOpen) => !isOpen && onClose()} open={open}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 z-50 w-full max-w-2xl -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background p-6 shadow-lg">
          <Dialog.Title className="font-medium text-base">{fileName}</Dialog.Title>
          ...
          <div className="mt-4">
            ...
            {content !== null && (
              <ScrollArea className="max-h-96">
                <pre className="whitespace-pre-wrap break-all rounded-md bg-muted p-3 font-mono text-sm">{content}</pre>
              </ScrollArea>
            )}
          </div>
