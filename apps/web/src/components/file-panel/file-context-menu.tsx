...
function MenuItem({ disabled, icon, label, onClick, variant = 'default' }: MenuItemProps) {
  return (
    <button
...
export function FileContextMenu({
  ...
  return (
    ...
    >
      <MenuItem icon={<FolderOpen className="h-4 w-4" />} label="Open" onClick={() => handleAction(onOpen)} />
      {!entry.isDir && (
        <MenuItem icon={<Eye className="h-4 w-4" />} label="Preview" onClick={() => handleAction(onPreview)} />
      )}
      {!entry.isDir && (
