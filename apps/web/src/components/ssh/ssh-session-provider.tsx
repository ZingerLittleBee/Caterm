...
export function SshSessionProvider({ children }: { children: ReactNode }) {
  const [sessions, setSessions] = useState<Map<string, SshSessionInfo>>(() => new Map())
  ...
  const updateSessionStatus = useCallback((sessionId: string, status: SshSessionStatus) => {
    setSessions((prev) => {
  ...
  const connect = useCallback(
    async (params: ConnectParams): Promise<string> => {
      ...
      const unlistenReconnecting = await listen(`ssh-reconnecting-${sessionId}`, () => {
        updateSessionStatus(sessionId, 'reconnecting')
      })

      // Listen for reconnected events.
      const unlistenReconnected = await listen(`ssh-reconnected-${sessionId}`, () => {
        updateSessionStatus(sessionId, 'connected')
      })

      // Listen for disconnect events.
      const unlistenDisconnect = await listen(`ssh-disconnect-${sessionId}`, () => {
