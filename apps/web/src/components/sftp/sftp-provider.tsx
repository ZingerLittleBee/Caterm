...
export function SftpProvider({ children }: { children: ReactNode }) {
	...
	const [activeSftpSessionId, setActiveSftpSessionId] = useState<string | null>(
		null
	);
	...
	const openStandalone = useCallback(
		async (params: {
			...
			const unlistenProgress = await listen<TransferTaskInfo>(
				`sftp-transfer-progress-${sessionId}`,
				(event) => {
					setTransfers((prev) => {
	...
	const chmod = useCallback(
		async (sessionId: string, path: string, mode: number): Promise<void> => {
	...
	const writeFile = useCallback(
		async (sessionId: string, path: string, content: string): Promise<void> => {
