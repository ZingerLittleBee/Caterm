    ...
    <Dialog.Root onOpenChange={(isOpen) => !isOpen && onClose()} open={open}>
      <Dialog.Portal>
        ...
        <Dialog.Popup className="fixed top-1/2 left-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-xl border bg-background p-6 shadow-lg">
          <Dialog.Title className="font-medium text-base">Permissions</Dialog.Title>
          <Dialog.Description className="mt-1 text-muted-foreground text-sm">{path}</Dialog.Description>
            ...
            {groups.map((group) => (
              <div key={group}>
                ...
                <div className="mt-1 flex items-center gap-4">
                  {PERMISSION_BITS.filter((p) => p.group === group).map((perm) => (
                    <span className="flex items-center gap-1.5 text-sm" key={perm.bit}>
                      <Checkbox
                        checked={hasBit(mode, perm.bit)}
                        onCheckedChange={(checked) => handleBitToggle(perm.bit, checked === true)}
                      />
                      {perm.label}
                    </span>
                  ))}
