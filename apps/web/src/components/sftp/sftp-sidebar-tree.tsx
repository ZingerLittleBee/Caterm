...
function updateNodeAtPath(nodes: TreeNode[], path: string, updater: (node: TreeNode) => TreeNode): TreeNode[] {
  ...
  return (
    <div className="flex h-full flex-col border-l">
      <div className="flex items-center justify-between border-b px-3 py-2">
        ...
        <Button disabled={rootLoading} onClick={loadRoot} size="icon" variant="ghost">
          <RefreshCw className={rootLoading ? 'h-4 w-4 animate-spin' : 'h-4 w-4'} />
        </Button>
      </div>
      <ScrollArea className="flex-1">
        {rootLoading && nodes.length === 0 ? (
          ...
          <div className="py-1">
            {nodes.map((node) => (
              <TreeNodeRow depth={0} key={node.entry.path} node={node} onToggle={handleToggle} />
            ))}
          </div>
        )}
      </ScrollArea>
        ...
        {isDir ? (
          <>
            {node.loading ? (
              ...
              <ChevronRight className={`h-4 w-4 shrink-0 transition-transform ${node.expanded ? 'rotate-90' : ''}`} />
            )}
            {node.expanded ? (
      ...
      {isDir && node.expanded && node.children && (
        <div>
          {node.children.map((child) => (
            <TreeNodeRow depth={depth + 1} key={child.entry.path} node={child} onToggle={onToggle} />
          ))}
        </div>
      )}
    </>
